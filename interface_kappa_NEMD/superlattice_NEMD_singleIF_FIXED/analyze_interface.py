...
def parse_reservoir_bounds(folder):
    ...
    and sink regions from the GROUP BOUNDARIES table, so plot shading
    reflects the real reservoir size instead of an approximation derived
    from the (length-dependent) mid-bin spacing."""
...
def compute_folder(folder):
    ...
    # Temperature  — keep full T_all for scatter plot
    ...
    return dict(
        ...
        T_all=T_all,           # ← full time-series for scatter
...
# ============================================================
# PAPER-STYLE PLOT FUNCTION
# ============================================================
def plot_paper_style(v):
    """Reproduce the paper-style temperature profile (dense scatter + extended fit lines)."""
    f          = v['folder']
    x_all_nm   = v['x_all_nm']
    T_all      = v['T_all']          # shape (n_timesteps, N_BINS)
    T_mean     = v['T_mean']
    x_iface_nm = v['x_iface_m'] * 1e9
    L_nm       = v['L_nm']
    bin_w      = v['L_bin_nm']

    # Real physical extent of the thermostatted source/sink regions (from
    # nemd_setup.txt), not an approximation from mid-bin spacing — that
    # approximation scaled with L and, after left-edge clamping, made the
    # source/sink bands visibly different widths at large L even though the
    # actual reservoirs are ~5 nm on both sides for every length here.
    src_bounds = v.get('src_bounds_nm')
    snk_bounds = v.get('snk_bounds_nm')
    margin = 0.03 * L_nm  # symmetric proportional breathing room, both edges
    ...
    if src_bounds and snk_bounds:
        ...
        # fallback to the old bin_w-based approximation if bounds weren't parsed
        bin_w = v['L_bin_nm']
    ...
    # ── colours ──────────────────────────────────────────────────────────────
    C_SCATTER = '#f5a623'   # orange dots  (per-timestep)
    C_FIT     = '#1a3a8f'   # dark blue    (fit lines, both sides like paper)
    C_SOURCE  = '#c0392b'   # red   – heat source
    C_SINK    = '#2471a3'   # blue  – heat sink

    fig, ax = plt.subplots(figsize=(6.5, 5.5))

    # 1. shaded bands for heat source / sink — now the real reservoir extents
    ...
    # 7. vertical "Heat source" / "Heat sink" labels — centered in the middle
    #    of each shaded band (not at its edge) so they clear the fit line and
    #    stay inside the axes / figure bounding box
    ...
    # 9. heat-flux arrow in bulk — x-position scaled to domain width (stays clear
    #    of the fit line at every L), y pinned near the interface temperature
    #    (~289 K, consistent across lengths) so it sits below the line, not on it
    ...
    # 10. axes — no title, journal style
...
# Sort by L_nm for a cleaner FSC / summary
...
# ============================================================
# PAPER-STYLE TEMPERATURE PROFILE PLOTS (one per folder)
# ============================================================
def savefig(name):
...
# ============================================================
# SUMMARY PLOT (multi-folder only) — the 3 requested panels
# ============================================================
if len(results) >= 2:
